...
router.get("/", async (req, res) => {
  try {
    const prints = await Print.find({ deletedAt: null })
...
router.post("/", auth, async (req, res) => {
  ...
  const imgObj = { title, original, price, dimensions, position, img }
...
router.patch("/:_id", auth, async (req, res) => {
  ...
  try {
    await Print.findOneAndUpdate({ _id }, { $set: { title, price, original, dimensions, position } })
...
router.delete("/:_id", auth, async (req, res) => {
  ...
  try {
    await Print.updateOne({ _id }, { deletedAt: Date.now() })
