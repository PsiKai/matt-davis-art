var printModel = require("../models/prints")

const artwork = (items) => {
    var artworkCards = ""
    items.forEach(item => {
        const { width, height } = JSON.parse(item.dimensions)
        var string = 
        `<div style='margin: 0.5rem; border: 1px solid black; padding: 0 1rem; border-radius: 0.5rem; background: lightgrey; max-width: 500px; display: inline-block;'>
            <h4>${item.title}</h4>
            <img src=${item.img} alt=${item.title} style="height: 300px; max-width: 100%; object-fit: cover;"/>
            <p>Size: ${width}" x ${height}"</p>
        </div>`
        artworkCards = artworkCards.concat(string)

        if (item.original === true) {
            printModel.findOneAndUpdate({title: item.title}, {soldOut: true}, err => {
                if (err) {
                    console.log("The artwork stock was not updated in the database because of an error");
                } else {
                    console.log("Artwork sold!");
                } 
            })
        }
    })

    return artworkCards
}

module.exports = { artwork }