...
const ImagePreview = ({
  ...
  return (
    <div className="img-preview">
      <div
        className={`image__wrapper ${src ? "" : "no-src"}`}
        onMouseOver={getOptimumHeight}
        onTouchStart={getOptimumHeight}
        onDragOver={e => {
          dragElement.current.dispatchEvent(new MouseEvent("drag", e))
          e.preventDefault()
        }}
      >
        <TransitionGroup>
          <CSSTransition key={transitionKey} timeout={400} classNames="crossfade">
            <>
        ...
        <div
          ...
          draggable
          onDrag={clickOverlay}
          onDragStart={e => e.dataTransfer.setDragImage(dragPreview, 0, 0)}
